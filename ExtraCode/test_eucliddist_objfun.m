...
if lambda == 150
    x = load([matdir filesep 'MRx3_l150_geneinds.mat'], 'mrmrinds');
    preloadinds = x.mrmrinds;
elseif (length(ng_param_list) > 5) && strcmp(method,'MRx3') % heuristic criterion
    preloadinds = MRx3_Selector(genevct,voxvgene,testnG(end),lambda);
else
    preloadinds = [];
end
...
save([matdir filesep 'cbc_colamd_tasic.mat'],'outstruct','elbowind');          
...
title('Recluster Error vs. nG Elbow Curve, \lambda = 150','FontSize',18);
xlabel('Norm. nG','FontSize',18);
ylabel('Norm. Error','FontSize',18);
legend({sprintf('Elbow nG = %d', outstruct(elbowind).nGen)});
